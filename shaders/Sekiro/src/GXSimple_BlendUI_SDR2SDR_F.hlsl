...
SamplerState PointSampler : register(s0); // can't disambiguate
//SamplerComparisonState PointSampler : register(s0); // can't disambiguate
...
OutputStruct ps_main(in InputStruct IN)
{
	...
	float4 r0 = HDRScene.Sample(PointSampler, xMirrorSample.xy );
	float4 r1 = UIScene.Sample(PointSampler, IN.param2.xy );
	
	r0.xyz = r0.xyz * r1.www + r1.xyz;
	r0.xyz = log(r0.xyz);
	r0.xyz = r0.xyz * outputGammaForSDR.xxxx;
